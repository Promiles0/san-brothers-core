...
function ProfilePage() {
  ...
  const uploadAvatar = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { error: dbErr } = await supabase.from("users").update({ profile_picture_url: pub.publicUrl }).eq("id", user.id);
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      <Card>
        <CardHeader><CardTitle className="text-base">{t("dashboard.profile.picture")}</CardTitle></CardHeader>
        <CardContent>
          <div className="flex items-center gap-4">
            <Avatar className="h-16 w-16">
              {profile.profile_picture_url ? <AvatarImage src={profile.profile_picture_url} /> : null}
            ...
            <label className="cursor-pointer">
              <Button asChild variant="outline" size="sm" disabled={avatarUploading}>
                <span>{avatarUploading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Upload className="mr-1 h-3.5 w-3.5" />}{t("dashboard.profile.changePicture")}</span>
              </Button>
      ...
      <Card>
        <CardHeader><CardTitle className="text-base">{t("dashboard.profile.info")}</CardTitle></CardHeader>
        <CardContent className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2"><Label>{t("dashboard.profile.fullName")}</Label><Input value={fullName} onChange={(e) => setFullName(e.target.value)} /></div>
            <div className="space-y-2"><Label>{t("dashboard.profile.phone")}</Label><Input value={phone} onChange={(e) => setPhone(e.target.value)} /></div>
            ...
            <div className="space-y-2"><Label>{t("dashboard.profile.tin")}</Label><Input value={tin} onChange={(e) => setTin(e.target.value)} /></div>
            <div className="space-y-2">
              <Label>{t("dashboard.profile.createdAt")}</Label>
              <Input value={new Date(profile.created_at).toLocaleDateString()} readOnly className="bg-muted/40" />
            </div>
          </div>
          <div className="space-y-2">
            <Label>{t("dashboard.profile.preferredLang")}</Label>
            <RadioGroup value={preferredLanguage} onValueChange={(v) => setPreferredLanguage(v as Locale)} className="flex gap-4">
          ...
          <Button onClick={save} disabled={saving}>{saving ? t("dashboard.common.saving") : t("dashboard.common.save")}</Button>
        </CardContent>
      </Card>
    </div>
  );
