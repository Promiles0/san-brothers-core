...
function Page() {
  ...
  useEffect(() => { if (!isLoading && !hasCapability("view_financial_reports")) navigate({ to: "/staff" }); }, [isLoading, hasCapability, navigate]);
  ...
  useEffect(() => {
    (async () => {
      ...
      const { count } = await supabase.from("service_requests").select("id", { count: "exact", head: true })
        .eq("status", "completed").gte("completed_at", start.toISOString());
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <Stat title="Cases Completed This Month" value={completedThisMonth === null ? "…" : String(completedThisMonth)} />
