...
interface ClaimDetail {
  id: string; status: string; reason_category: string; description: string;
  evidence_file_paths: string[] | null; resolution_notes: string | null;
  refund_amount_rwf: number | null; service_request_id: string | null;
  client: { full_name: string | null; email: string; phone: string | null } | null;
  service_request: { id: string; service_category: string; service: { name_en: string } | null } | null;
}
...
function Page() {
  ...
  useEffect(() => { if (!capLoading && !hasCapability("handle_claims")) navigate({ to: "/staff" }); }, [capLoading, hasCapability, navigate]);
  ...
  const load = async () => {
    ...
    const { data, error } = await supabase.from("claims")
      .select("id,status,reason_category,description,evidence_file_paths,resolution_notes,refund_amount_rwf,service_request_id,client:users!claims_client_id_fkey(full_name,email,phone),service_request:service_requests(id,service_category,service:services(name_en))")
      .eq("id", id).single();
    if (error) { toast.error(error.message); setLoading(false); return; }
    const row = data as unknown as ClaimDetail;
    setC(row); setStatus(row.status); setNotes(row.resolution_notes ?? ""); setRefund(row.refund_amount_rwf?.toString() ?? "");
    setLoading(false);
  };
  ...
  const save = async () => {
    if ((status === "resolved" || status === "rejected") && !notes.trim()) {
      toast.error("Resolution notes required"); return;
    }
    const patch: Record<string, unknown> = {
      status, resolution_notes: notes || null,
  ...
  if (loading || !c) return <div className="space-y-3"><Skeleton className="h-8 w-64" /><Skeleton className="h-64 w-full" /></div>;

  return (
    <div className="space-y-4">
      <Button asChild variant="ghost" size="sm"><Link to="/staff/claims"><ArrowLeft className="h-4 w-4" /> Back</Link></Button>
      ...
      <Card><CardHeader><CardTitle className="text-base">Details</CardTitle></CardHeader>
        <CardContent className="space-y-2 text-sm">
          <p><span className="text-muted-foreground">Reason: </span>{c.reason_category.replace(/_/g, " ")}</p>
        ...
        <CardContent className="space-y-1 text-sm">
          ...
          {c.service_request && (
            <Button asChild size="sm" variant="link" className="px-0">
              <Link to={`/staff/${c.service_request.service_category}/${c.service_request.id}`}>
      ...
      <Card><CardHeader><CardTitle className="text-base">Resolution</CardTitle></CardHeader>
        <CardContent className="space-y-3">
          ...
          <div className="space-y-1"><Label>Resolution notes</Label><Textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={4} /></div>
          <div className="space-y-1">
            <Label>Refund amount (RWF)</Label>
            <Input type="number" value={refund} onChange={(e) => setRefund(e.target.value)} placeholder="0" />
            <p className="text-xs text-muted-foreground">This records the refund amount. Process the actual transfer manually.</p>
          </div>
