...
const GROUP_STYLES = {
  ...
  visa: {
    Icon: Plane,
    color: "text-visa",
    open: "data-[state=open]:text-visa",
    border: "border-l-visa",
    tint: "bg-visa/10",
  },
  accounting: {
    Icon: Calculator,
    color: "text-accounting",
    open: "data-[state=open]:text-accounting",
    border: "border-l-accounting",
    tint: "bg-accounting/10",
  },
  translation: {
    Icon: Languages,
    color: "text-translation",
    open: "data-[state=open]:text-translation",
    border: "border-l-translation",
    tint: "bg-translation/10",
  },
  payments: {
    Icon: CreditCard,
    color: "text-consultancy",
    open: "data-[state=open]:text-consultancy",
    border: "border-l-consultancy",
    tint: "bg-consultancy/10",
  },
};
      ...
      <PageHero title={t("faq.heroTitle")} subtitle={t("faq.heroSubtitle")}>
        <div className="max-w-xl">
          <div className="relative">
            ...
            <Input
              className="h-12 rounded-full border-border bg-background pl-11 shadow-sm transition-shadow focus-visible:shadow-lg"
            ...
            {groups.map(({ key, group }) => {
              ...
              return (
                <a
                  key={key}
                  href={`#${key}`}
                  className="flex items-center gap-2 rounded-full border border-border bg-card px-3 py-2 text-sm text-foreground"
                >
        ...
        <div className="grid gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(0,3fr)]">
          <aside className="hidden lg:block">
            <nav className="sticky top-24 space-y-2" aria-label="FAQ topics">
              <h2 className="mb-4 text-sm font-bold uppercase tracking-[0.16em] text-foreground">
                Browse by Topic
              </h2>
              {groups.map(({ key, group }) => {
                ...
                return (
                  <a
                    key={key}
                    href={`#${key}`}
                    className={`group flex items-center gap-3 rounded-lg border border-border border-l-4 ${border} bg-card px-3 py-3 text-sm text-foreground transition hover:bg-muted`}
                  >
                    <Icon className={`h-4 w-4 ${color}`} />
                    <span className="flex-1">{group.title}</span>
                    <span className="rounded-full bg-muted px-2 py-0.5 text-xs text-muted-foreground group-hover:bg-background">
                      {group.items.length}
                    </span>
                  </a>
                );
              ...
              filteredGroups.map(({ key, group, items }) => {
                ...
                return (
                  <section
                    id={key}
                    key={key}
                    className="reveal scroll-mt-28 border-b border-border pb-12"
                  >
                    ...
                    <Accordion type="single" collapsible className="mt-5">
                      {items.map((it) => (
                        <AccordionItem key={it.q} value={it.q} className="border-border">
                          <AccordionTrigger
                            className={`text-base font-semibold text-foreground hover:no-underline ${open}`}
                          >
                            {it.q}
                          </AccordionTrigger>
                          <AccordionContent
                            className={`border-l-2 ${border} pl-4 pr-2 text-sm leading-relaxed text-muted-foreground`}
                          >
                            {it.a}
                          </AccordionContent>
                        </AccordionItem>
