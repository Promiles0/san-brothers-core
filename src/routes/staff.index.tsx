  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const sel =
          "id,status,created_at,updated_at,assigned_staff_id,service_category,client:users(full_name),service:services(name_en)";
        ...
        const mapRow = (r: Record<string, unknown>): Row => ({
          ...(r as unknown as Row),
          basePath: basePathFor(r.service_category as string),
        });
              ...
              <ul className="space-y-2">
                {pending.map((r) => (
                  <li
                    key={r.id}
                    className={cn(
                      "flex items-center justify-between gap-2 rounded-md border-l-4 bg-muted/30 p-2 text-sm",
                      r.status === "awaiting_client" ? "border-orange-500" : "border-yellow-500",
                    )}
                  >
                    <div className="min-w-0">
                      <p className="truncate font-medium">
                        {r.client?.full_name ?? "—"} — {r.service?.name_en}
                      </p>
                      <StatusBadge status={r.status} />
                    </div>
                    <Link
              ...
              {week.map((d) => {
                ...
                return (
                  <div
                    key={key}
                    className={cn(
                      "min-h-32 rounded border p-1 text-xs",
                    ...
                    <ul className="space-y-1">
                      {dayRows.slice(0, 4).map((r) => (
                        <li key={r.id}>
                          <Link
                            to={`${r.basePath}/${r.id}` as never}
                            className="block truncate rounded bg-muted px-1 py-0.5 hover:bg-muted-foreground/10"
                          >
                            {r.client?.full_name ?? "—"}
                          </Link>
                        </li>
                      ...
                      {dayRows.length > 4 && (
                        <li className="text-muted-foreground">+{dayRows.length - 4}</li>
                      )}
                    </ul>
                  </div>
                );
