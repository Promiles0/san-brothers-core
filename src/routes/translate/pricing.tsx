...
function PlanCard({ plan, ctaLabel }: { plan: PricingPlan; ctaLabel: string }) {
  const { t } = useI18n();
  const formatMin = plan.minutes >= 1000
    ? `${plan.minutes.toLocaleString()} ${t("translate.pricing.minutes")}`
    : `${plan.minutes} ${t("translate.pricing.minutes")}`;
  const validityLabel = plan.validity
    ? t(`translate.pricing.validity.${plan.validity}`)
    : "";
  return (
    <Card className={plan.isPopular ? "relative border-accent shadow-lg ring-2 ring-accent/30" : "relative"}>
      {plan.isPopular && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2">
          <Badge className="bg-accent text-accent-foreground hover:bg-accent">{t("translate.pricing.mostPopular")}</Badge>
        </div>
      )}
      <CardContent className="flex flex-col gap-4 p-6">
        <div className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">
          {plan.isPopular ? t("translate.pricing.tierPopular") : t(`translate.pricing.tiers.${plan.name.toLowerCase().replace(/\s+/g, "")}`) || plan.name}
        </div>
        ...
        <div className="text-sm font-medium">
          {formatMin}{validityLabel ? ` / ${validityLabel}` : ""}
        </div>
    ...
    <TranslateLayout>
          ...
          {groups.map((g) => (
            <TabsContent key={g.key} value={g.key} className="mt-10">
              <div className="grid gap-6 md:grid-cols-3">
                {pricingPlans.filter((p) => p.planType === g.key).map((p, i) => (
      ...
      <section className="border-y border-border bg-muted/30">
        <div className="mx-auto max-w-6xl px-4 py-16 md:px-6">
          <h2 className="text-2xl font-bold tracking-tight md:text-3xl">{t("translate.pricing.docHeading")}</h2>
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            <Card>
              <CardContent className="p-6">
                <h3 className="text-base font-semibold">{t("translate.pricing.docStandard.title")}</h3>
                <p className="mt-1 text-2xl font-bold">5,000 RWF <span className="text-sm font-normal text-muted-foreground">/ {t("translate.pricing.page")}</span></p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-6">
                <h3 className="text-base font-semibold">{t("translate.pricing.docCertified.title")}</h3>
                <p className="mt-1 text-2xl font-bold">12,000 RWF <span className="text-sm font-normal text-muted-foreground">/ {t("translate.pricing.page")}</span></p>
              </CardContent>
            </Card>
          </div>
      ...
      <section className="mx-auto max-w-3xl px-4 py-16 md:px-6">
        <h2 className="text-2xl font-bold tracking-tight md:text-3xl">{t("translate.pricing.faq.heading")}</h2>
        <div className="mt-6 space-y-4">
          {["expire", "cancel", "payments"].map((k) => (
            <Card key={k}>
              <CardContent className="p-5">
                <h3 className="font-semibold">{t(`translate.pricing.faq.${k}.q`)}</h3>
                <p className="mt-1 text-sm text-muted-foreground">{t(`translate.pricing.faq.${k}.a`)}</p>
              </CardContent>
            </Card>
          ))}
        </div>
