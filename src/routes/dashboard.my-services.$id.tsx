...
function ServiceDetailPage() {
  ...
  const load = async () => {
    const [srRes, docRes] = await Promise.all([
      supabase.from("service_requests").select("id,status,progress_step,progress_total,notes,created_at,services(name_en,name_zh,name_rw)").eq("id", id).maybeSingle(),
      supabase.from("documents").select("*").eq("service_request_id", id).order("uploaded_at", { ascending: false }),
    ]);
    if (srRes.error) { toast.error(srRes.error.message); setSr(null); return; }
  ...
  if (sr === undefined) return <div className="space-y-3"><Skeleton className="h-8 w-64" /><Skeleton className="h-40" /></div>;
  if (!sr) return <div className="text-center py-12"><p className="text-muted-foreground">{t("dashboard.myServices.notFound")}</p></div>;

  const localName = (locale === "zh" && sr.services?.name_zh) || (locale === "rw" && sr.services?.name_rw) || sr.services?.name_en || "";
  ...
  const upload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { error: docErr } = await supabase.from("documents").insert({
        service_request_id: id, client_id: user.id, uploaded_by: user.id,
        file_path: path, file_name: f.name, file_type: f.type || null, file_size_bytes: f.size, status: "uploaded",
      });
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <Button asChild variant="ghost" size="sm"><Link to="/dashboard/my-services"><ArrowLeft className="mr-1 h-4 w-4" /> {t("dashboard.common.back")}</Link></Button>

      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">{localName}</h1>
          <p className="text-xs text-muted-foreground">{t("dashboard.common.created")}: {new Date(sr.created_at).toLocaleDateString()}</p>
        </div>
      ...
      {/* Timeline */}
      <Card>
        <CardHeader><CardTitle className="text-base">{t("dashboard.myServices.progress")}</CardTitle></CardHeader>
        <CardContent>
          <ol className="flex flex-col gap-4 md:flex-row md:items-center md:gap-2">
            {STEPS.map((s, i) => {
              ...
              return (
                <li key={s} className="flex items-center gap-2 md:flex-col md:text-center md:flex-1">
                  <div className={cn(
        ...
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          ...
          <label className="cursor-pointer">
            <Button asChild size="sm" variant="outline" disabled={uploading}>
              <span><Upload className="mr-1 h-3.5 w-3.5" /> {uploading ? t("dashboard.common.uploading") : t("dashboard.myServices.uploadAnother")}</span>
            </Button>
          ...
          {docs.length === 0 ? (
            ...
            <div key={d.id} className="flex flex-wrap items-center gap-3 rounded-md border border-border p-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {d.rejection_reason && <div className="mt-1 text-xs text-destructive">{d.rejection_reason}</div>}
              </div>
      ...
      {/* Chat stub */}
      <Card>
        <CardHeader><CardTitle className="text-base">{t("dashboard.myServices.chat")}</CardTitle></CardHeader>
        <CardContent className="space-y-3">
          <div className="space-y-2 max-h-40 overflow-y-auto">
            <div className="rounded-lg bg-muted px-3 py-2 text-sm max-w-xs">{t("dashboard.myServices.chatPlaceholder")}</div>
          </div>
          ...
          <p className="text-xs text-muted-foreground">{t("dashboard.myServices.chatComingSoon")}</p>
        </CardContent>
      </Card>

      <div className="flex justify-end">
        <Button asChild variant="outline" size="sm">
          <Link to="/dashboard/claims/new" search={{ service_request_id: id } as never}>{t("dashboard.myServices.openClaim")}</Link>
        </Button>
      </div>
    </div>
  );
