...
const items: Item[] = [
  { label: "Home", icon: LayoutDashboard, to: "/staff" },
  {
    ...
    to: "/staff/visa",
    cap: "handle_visa",
    badgeKey: "unassignedVisa",
  },
  {
    label: "Accounting",
    icon: Calculator,
    to: "/staff/accounting",
    cap: "handle_accounting",
    badgeKey: "unassignedAccounting",
  },
      ...
      try {
        const [visa, acc, claims] = await Promise.all([
          supabase
            ...
            .eq("service_category", "visa")
            .not("status", "in", "(completed,rejected,cancelled)"),
          supabase
            .from("service_requests")
            .select("id", { count: "exact", head: true })
            .is("assigned_staff_id", null)
            .eq("service_category", "accounting")
            .not("status", "in", "(completed,rejected,cancelled)"),
          supabase
            .from("claims")
            .select("id", { count: "exact", head: true })
...
export function StaffSidebar({
  ...
  const renderItem = (item: Item) => {
    ...
    return (
      <button key={item.label} onClick={() => handleClick(item)} className={cls}>
        {inner}
      </button>
    );
  };
